...
pdata. <- pdata. |> mutate(geotile = paste('ll',floor(x/5)*100,floor(y/5)), elzone = floor(((elev/500)+0.1)^1*1))|> group_by(geotile, elzone) |> mutate(wts=100/length(ID)) |> ungroup()
trdata. <- trdata. |> mutate(geotile = paste('ll',floor(x/5)*100,floor(y/5)), elzone = floor(((elev/500)+0.1)^1*1))|> group_by(geotile, elzone) |> mutate(wts=100/length(ID)) |> ungroup()
tdata <- tdata |> mutate(geotile = paste('ll',floor(x/5)*100,floor(y/5)), elzone = floor(((elev/500)+0.1)^1*1))|> group_by(geotile, elzone) |> mutate(wts=100/length(ID)) |> ungroup()
...
# thissample <- spatSample(br1, 5000) |> subset(w50 <= 0.6, select=c("lat","coslon","sinlon","elev","w50","w500","w5000","relev","wind315","wind000","wind045","wind090","wind135","windmax","windmin","windmean"))
...
# 
# 
# globl <- globl |> cbind(predict(myPCA, st_drop_geometry(globl[,colnames(thissample)])))
# globl <- globl |> mutate(cat1 = case_when(PC1 >= myPCA.x$p1.8 ~'A',
#                                           PC1 >= myPCA.x$p1.6 ~'B',
#                                           PC1 >= myPCA.x$p1.4 ~'C',
#                                           PC1 >= myPCA.x$p1.2 ~'D',
#                                           TRUE ~'E'),
...
train <- train |> mutate(near = ifelse(x >= ex[1] & x <= ex[2] & y >= ex[3] & y <= ex[4],1,0)) |> mutate(wts = ifelse(near %in% 1, 100*wts,1*wts))
...
# st_write(pdata.,'global/pdata.shp')
# library(ranger)
# rf = ranger(z~(coslon)+(sinlon)+(lat)+(elev)+(relev)+
#               (w500)+(w5000)+
#               windmax+windmin+windmean+
#               (wind000)+(wind090)+(wind180)+(wind270)+
#               (wind045)+(wind135)+(wind225)+(wind315)
#             ,data=as.data.frame(train), case.weights = train$wts)
# mod <- predict(mbr, rf, na.rm=T, type = "response")
# plot(mod)
# train <- train |> mutate(pred = predictions(predict(rf, as.data.frame(train), na.rm=T)), resid = z-(pred))

if((exa[2]-exa[1])*(exa[4]-exa[3])<3500){
