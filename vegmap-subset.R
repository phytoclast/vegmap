...
sand <- rast(paste0(path2, 'sand.tif')); names(sand)<-'sand'
SoilpH <- rast(paste0(path2, 'SoilpH.tif')); names(SoilpH)<-'SoilpH'
hydric <- rast(paste0(path2, 'hydric.tif')); names(hydric)<-'hydric'
salids <- rast(paste0(path2, 'salids.tif')); names(salids)<-'salids'
...
bedrock <- rast(paste0(path2, 'bedrock.tif')); names(bedrock)<-'bedrock'
clay <- rast(paste0(path2, 'clay.tif')); names(clay)<-'clay'
...
r.df$wt <- ifelse(is.na(r.df$kuchler.tree), 1,10)
...
rf <- ranger(wet ~ 
             ...
             ,
             data=r.df.s, num.trees=50, sample.fraction = 0.02, max.depth = 12, importance = 'impurity', write.forest = TRUE, case.weights = r.df.s$wt)
...
rf <- ranger(veg ~ 
             ...
             ,
             data=r.df.s, num.trees=100, sample.fraction = 0.01, max.depth = 18, importance = 'impurity', write.forest = TRUE, case.weights = r.df.s$wt)
r.df$output <- predictions(predict(rf, data=r.df))
...
rf <- ranger(ever ~ 
             ...
             +wetmodel+vegmodel+woodmodel+treemodel
             ,
             data=r.df.s, num.trees=100, sample.fraction = 0.02, max.depth = 18, importance = 'impurity', write.forest = TRUE, case.weights = r.df.s$wt)
