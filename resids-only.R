...
for(mo in c(1:12)){#mo=9
  
  nameofpar <- paste0(parm,mos[mo])
  filnam <- paste0('clim1990/global/',nameofpar,'.tif')
  ...
  smoothdata <- train |> st_drop_geometry() |> 
    mutate(xgeo = paste(floor(x*10),'-',floor(y*10))) |> group_by(xgeo) |> summarise(x=mean(x), y=mean(y), elev=mean(elev), windmean=mean(windmean), z=mean(z),resid=mean(resid)) |> ungroup() 
  
  xyz <-train[,c('x','y','resid')] |> st_drop_geometry()
  gs <- gstat(formula=resid~1, locations=~x+y, data=xyz, nmax=32, set=list(idp = 2))
  ...
  writeRaster(z1, filnam, overwrite=T)
